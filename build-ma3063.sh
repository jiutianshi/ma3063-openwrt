...
# ---------------------------------------------------------------------------
# GATE M: the injected macros must be *constant expressions*.
#
# Why this gate exists: upstream's version of the 907 patch is written against a
# 6.x kernel and uses FIELD_PREP_CONST(), which only appeared in v6.6.  This
# tree is backports-6.1.24 on a 5.15.150 kernel, so the macro does not exist and
# the build died with
#   hw.h:88:28: error: implicit declaration of function 'FIELD_PREP_CONST'
#   hw.h:88:28: error: initializer element is not constant
# after ~50 minutes.  FIELD_PREP() is not a substitute either: it expands to a
# GCC statement expression and is therefore illegal in the `static const struct
# ath11k_hw_regs` initialisers.
#
# The macro text is lifted out of the patched hw.h (not hard-coded), and the
# snippet reproduces the three contexts the real code uses the macros in:
# a static-const initialiser, an arithmetic expression, and a switch-case label.
# Any helper that is not a plain constant expression fails here in seconds.
# ---------------------------------------------------------------------------
MCHK=/tmp/ma3063-macrocheck.c
MCHK_BIN=/tmp/ma3063-macrocheck
{
  echo '/* auto-generated by build-ma3063.sh (GATE M) */'
  echo '#include <stdio.h>'
  # GENMASK as 5.15 defines it, so the host compiler needs no kernel headers
  echo '#define GENMASK(h, l) (((~0UL) - (1UL << (l)) + 1) & (~0UL >> (63 - (h))))'
  sed -n '/#define ATH11K_REG_TYPE_MASK/,/#define ATH11K_REG_OFFSET_MASK/p' "$ATH11K_DIR/hw.h"
  echo 'struct mchk_regs { unsigned int v; };'
  echo 'static const struct mchk_regs mchk_normal = { .v = ATH11K_REG_TYPE_NORMAL + 0x1000 };'
  echo 'static const struct mchk_regs mchk_ce = { .v = ATH11K_REG_TYPE_CE + 0x08400000 - 0x08400000 };'
  echo 'static int mchk_dispatch(unsigned int offset) {'
  echo '  switch (offset & ATH11K_REG_TYPE_MASK) {'
  echo '  case ATH11K_REG_TYPE_NORMAL: return 1;'
  echo '  case ATH11K_REG_TYPE_CE: return 2;'
  echo '  default: return 0; } }'
  echo 'int main(void) {'
  echo '  printf("%u %u %d\n", mchk_normal.v, mchk_ce.v, mchk_dispatch(ATH11K_REG_TYPE_CE));'
  echo '  return 0; }'
} > "$MCHK"

if ! gcc -Wall -Werror -o "$MCHK_BIN" "$MCHK" 2>/tmp/ma3063-macrocheck.err; then
  echo "GATE M FAIL: the injected register macros are not constant expressions" | tee -a build.log
  echo "------------- hw.h macro block as injected -------------" | tee -a build.log
  sed -n '/#define ATH11K_REG_TYPE_MASK/,/#define ATH11K_REG_OFFSET_MASK/p' "$ATH11K_DIR/hw.h" | tee -a build.log
  echo "------------- host gcc -------------" | tee -a build.log
  head -40 /tmp/ma3063-macrocheck.err | tee -a build.log
  exit 1
fi
MCHK_OUT="$("$MCHK_BIN")"
MCHK_EXPECT="$(( (0 << 28) | 0x1000 )) $(( (2 << 28) | 0 )) 2"
echo "GATE M OK: macros are constants -- normal/ce/dispatch = $MCHK_OUT (expected $MCHK_EXPECT)" | tee -a build.log
if [ "$MCHK_OUT" != "$MCHK_EXPECT" ]; then
  echo "GATE M FAIL: macro arithmetic is wrong (got '$MCHK_OUT', expected '$MCHK_EXPECT')" | tee -a build.log
  exit 1
fi
# and the CE tag must carry type 2 in the high nibble, i.e. be reachable as a
# CE-space offset after masking -- the whole point of the fix
if ! grep -q 'ATH11K_REG_TYPE_CE.*ATH11K_REG_TYPE(2)' "$ATH11K_DIR/hw.h"; then
  echo "GATE M FAIL: ATH11K_REG_TYPE_CE is no longer ATH11K_REG_TYPE(2)" | tee -a build.log
  exit 1
fi
...
# Surface the actual compiler errors into the small published trace.  build.log
# is ~30 MB and cannot be fetched through the GitHub API, so without this the
# only sign of a broken patch is "image unchanged" and the real cause has to be
# dug out of the error file by hand.
if [ "$rc4" -ne 0 ]; then
  echo "---- pass2 compile errors (last 40 matching lines) ----" | tee -a build.log
  grep -nE "error:|Error [0-9]+|No rule to make|undefined reference" build.log 2>/dev/null \
    | tail -40 | tee -a build.log
  echo "---- end of compile errors ----" | tee -a build.log
fi
...
if [ "$rc4" -ne 0 ]; then
  echo "ERROR: pass2 build FAILED (rc=$rc4) -- no new firmware was produced; refusing to publish" | tee -a build.log
  exit 1
fi
if [ "$GATE_FAIL" -ne 0 ]; then
  echo "ERROR: ath11k fix is not verified by the firmware gates -- refusing to publish this firmware" | tee -a build.log
  exit 1
fi
